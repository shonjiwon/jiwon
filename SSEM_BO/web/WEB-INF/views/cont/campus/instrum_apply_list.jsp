            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/instrum_apply_list_excel.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/campus/instrum_apply_list.do">
        ...
        <article class="contUnit sch_area">
            ...
            <div class="line">
                ...
                <div class="formCont">
                    <label for="searchKind3">악기유형</label>
                ...
                <div class="formCont">
                    ...
                    <select id="searchKind4" name="searchKind4" class="wid150">
                        <option value="">전체</option>
                        <option value="A" <c:if test="${searchKind4 eq 'A'}">selected="selected"</c:if>>신청</option>
                        <option value="R" <c:if test="${searchKind4 eq 'R'}">selected="selected"</c:if>>신청취소</option>
                        <option value="O" <c:if test="${searchKind4 eq 'O'}">selected="selected"</c:if>>승인</option>
                        <option value="C" <c:if test="${searchKind4 eq 'C'}">selected="selected"</c:if>>승인취소</option>
                    </select>
                </div>
            </div>
            <div class="line">
                <div class="formCont">
                    <label for="searchVal2">결재상태</label>
                    <select id="searchVal2" name="searchVal2" class="wid150">
                        <option value="">전체</option>
                        <option value="A" <c:if test="${searchVal2 eq 'A'}">selected="selected"</c:if>>결제대기</option>
                        <option value="B" <c:if test="${searchVal2 eq 'B'}">selected="selected"</c:if>>결제완료</option>
                        <option value="C" <c:if test="${searchVal2 eq 'C'}">selected="selected"</c:if>>환불신청</option>
                        <option value="D" <c:if test="${searchVal2 eq 'D'}">selected="selected"</c:if>>환불처리</option>
                    </select>
                </div>
                <div class="formCont">
                    <label for="searchVal3">반납완료</label>
                    <select id="searchVal3" name="searchVal3" class="wid150">
                        <option value="">전체</option>
                        <option value="Y" <c:if test="${searchVal3 eq 'Y'}">selected="selected"</c:if>>Y</option>
                        <option value="N" <c:if test="${searchVal3 eq 'N'}">selected="selected"</c:if>>N</option>
                    </select>
                </div>
                <div class="formCont">
                    <label for="searchVal4">연체여부</label>
                    <select id="searchVal4" name="searchVal4" class="wid150">
                        <option value="">전체</option>
                        <option value="Y" <c:if test="${searchVal4 eq 'Y'}">selected="selected"</c:if>>Y</option>
                        <option value="N" <c:if test="${searchVal4 eq 'N'}">selected="selected"</c:if>>N</option>
                    </select>
                </div>
            </div>
            <div class="line">
                <div class="formCont">
                    <label for="searchSDate">신청일</label>
                ...
                <select id="searchKind" name="searchKind" class="wid150">
                    ...
                    <option value="3" <c:if test="${searchKind eq '3'}">selected="selected"</c:if>>신청자</option>
                </select>
        ...
        <table>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    ...
                    <td>${item.instrum_name}</td>
                    <td>${item.apy_id}</td>
                    <td><a href="/ssemadmin/cont/campus/instrum_apply_view.do?instrum_apy_idx=${item.instrum_apy_idx}">${item.apy_nm}<c:if test="${item.apy_parent_name ne null}"><br/>(${item.apy_parent_name})</c:if></a></td>
                    <td>${item.apy_mob_no}<c:if test="${item.apy_parent_mob_no ne null}"><br/>(${item.apy_parent_mob_no})</c:if></td>
                    <td>${item.rental_qty}개</td>
                    <td>${item.rental_start_dt}<br/>~ ${item.rental_end_dt}</td>
                    <td>${fn:substring(item.reg_dt, 0, 10)}</td>
                    <td>
                        <c:choose>
                            <c:when test="${item.status eq 'A'}">신청</c:when>
                            <c:when test="${item.status eq 'R'}">신청취소</c:when>
                            <c:when test="${item.status eq 'O'}">승인</c:when>
                            <c:when test="${item.status eq 'C'}">승인취소</c:when>
                            <c:otherwise>-</c:otherwise>
                        </c:choose>
                    </td>
                    <td>
                        <c:choose>
                            <c:when test="${item.pay_status eq 'A'}">결제대기</c:when>
                            <c:when test="${item.pay_status eq 'B'}">결제완료</c:when>
                            <c:when test="${item.pay_status eq 'C'}">환불신청</c:when>
                            <c:when test="${item.pay_status eq 'D'}">환불처리</c:when>
                            <c:otherwise>-</c:otherwise>
                        </c:choose>
                    </td>
                    <td>
                        <c:choose>
                            <c:when test="${item.return_yn eq 'Y'}">Y</c:when>
                            <c:when test="${item.return_yn eq 'N'}">N</c:when>
                            <c:otherwise>-</c:otherwise>
                        </c:choose>
                    </td>
                    <td>
                        <c:choose>
                            <c:when test="${item.late_yn eq 'Y'}">Y<br/><span class="co_red">(+${item.late_day})</span></c:when>
                            <c:when test="${item.late_yn eq 'N'}">N</c:when>
                            <c:otherwise>-</c:otherwise>
                        </c:choose>
                    </td>
                </tr>
