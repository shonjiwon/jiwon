            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/person_reg_list_excel.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/campus/person_reg_list.do">
        ...
        <article class="contUnit sch_area">
            ...
            <div class="line">
                <div class="formCont">
                    <label for="searchKind2">분류</label>
            ...
            <div class="line">
                <div class="formCont">
                    ...
                    <select id="searchKind4" name="searchKind4" class="wid150">
                        <option value="">전체</option>
                        <option value="M" <c:if test="${searchKind4 eq 'M'}">selected="selected"</c:if>>남성</option>
                        <option value="F" <c:if test="${searchKind4 eq 'F'}">selected="selected"</c:if>>여성</option>
                    </select>
                </div>
                <div class="formCont">
                    <label for="searchVal2">연령대</label>
                    <select id="searchVal2" name="searchVal2" class="wid150">
                        <option value="">전체</option>
                        <option value="1" <c:if test="${searchVal2 eq '1'}">selected="selected"</c:if>>10대</option>
                        <option value="2" <c:if test="${searchVal2 eq '2'}">selected="selected"</c:if>>20대</option>
                        <option value="3" <c:if test="${searchVal2 eq '3'}">selected="selected"</c:if>>30대</option>
                        <option value="4" <c:if test="${searchVal2 eq '4'}">selected="selected"</c:if>>40대</option>
                        <option value="5" <c:if test="${searchVal2 eq '5'}">selected="selected"</c:if>>50대</option>
                        <option value="6" <c:if test="${searchVal2 eq '6'}">selected="selected"</c:if>>60대</option>
                        <option value="7" <c:if test="${searchVal2 eq '7'}">selected="selected"</c:if>>70대</option>
                        <option value="8" <c:if test="${searchVal2 eq '8'}">selected="selected"</c:if>>80대</option>
                    </select>
                </div>
                <div class="formCont">
                    <select id="searchKind" name="searchKind" class="wid150">
                        <option value="ALL">전체</option>
                        <option value="1" <c:if test="${searchKind eq '1'}">selected="selected"</c:if>>아이디</option>
                        <option value="2" <c:if test="${searchKind eq '2'}">selected="selected"</c:if>>신청자</option>
                    </select>
        ...
        <table>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    ...
                    <td>${item.reg_id}</td>
                    <td><a href="/ssemadmin/cont/campus/person_reg_view.do?person_book_reg_idx=${item.person_book_reg_idx}">${item.reg_name}</a></td>
                    <td>${item.reg_mob_no}</td>
                    <td>${item.reg_birth}</td>
                    <td>
                        <c:choose>
                            <c:when test="${item.reg_gender eq 'M'}">남자</c:when>
                            <c:when test="${item.reg_gender eq 'F'}">여자</c:when>
                            <c:otherwise>-</c:otherwise>
                        </c:choose>
                    </td>
                    <td>
                        <c:choose>
                            <c:when test="${item.reg_age eq '1'}">10대</c:when>
                            <c:when test="${item.reg_age eq '2'}">20대</c:when>
                            <c:when test="${item.reg_age eq '3'}">30대</c:when>
                            <c:when test="${item.reg_age eq '4'}">40대</c:when>
                            <c:when test="${item.reg_age eq '5'}">50대</c:when>
                            <c:when test="${item.reg_age eq '6'}">60대</c:when>
                            <c:when test="${item.reg_age eq '7'}">70대</c:when>
                            <c:when test="${item.reg_age eq '8'}">80대</c:when>
                            <c:otherwise>-</c:otherwise>
                        </c:choose>
                    </td>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${item.status eq 'A'}">신청대기</c:when>
                            <c:when test="${item.status eq 'P'}">신청확인</c:when>
                            <c:when test="${item.status eq 'O'}">인물도서 확정</c:when>
