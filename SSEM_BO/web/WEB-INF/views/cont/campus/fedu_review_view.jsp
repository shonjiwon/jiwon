                ...
                $.ajax({
                    ...
                    success: function(data){
                        if (data !== true) {
                            ...
                            alert("삭제하였습니다.");
                            location.href = "?future_edu_review_idx=${futureEduReviewVo.future_edu_review_idx}";
                        }
                    }
                });
            }else{
...
<div class="container">
    ...
    <article class="contUnit tbl_area leftType">
        ...
        <form name="delfrm" id="delfrm" action="/ssemadmin/cont/campus/fedu_review_delete.do" method="post" enctype="multipart/form-data">
            <input type="hidden" name="future_edu_review_idx" value="${futureEduReviewVo.future_edu_review_idx}" />
        </form>
        <form name="frm" id="frm" action="/ssemadmin/cont/campus/fedu_review_view.do" method="post" enctype="multipart/form-data">
            <input type="hidden" name="future_edu_review_idx" value="${futureEduReviewVo.future_edu_review_idx}" />
            <table>
                <colgroup>
                    <col style="width:15%">
                ...
                <tr>
                    <th>노출상태</th>
                    <td colspan="5">
                        <input type="radio" name="use_yn" id="use_y" value="Y" <c:if test="${futureEduReviewVo.use_yn eq 'Y'}">checked="checked"</c:if>>
                        <label for="use_y">노출</label>
                        <input type="radio" name="use_yn" id="use_n" value="N" <c:if test="${futureEduReviewVo.use_yn eq 'N'}">checked="checked"</c:if>>
                        <label for="use_n">미노출</label>
                    </td>
                </tr>
                <tr>
                    <th>소속</th>
                    <td>${futureEduReviewVo.p_name}</td>
                    <th>교육기관</th>
                    <td colspan="3">${futureEduReviewVo.grp_name}</td>
                </tr>
                <tr>
                    <th>작성자</th>
                    <td>${futureEduReviewVo.reg_id}</td>
                    <th>작성일</th>
                    <td>${futureEduReviewVo.reg_dt}</td>
                    <th>조회수</th>
                    <td>${futureEduReviewVo.cnt}</td>
                </tr>
                <tr>
                    <th>최종수정자</th>
                    <td>${futureEduReviewVo.up_id}</td>
                    <th>최종수정일</th>
                    <td colspan="3">${futureEduReviewVo.up_dt}</td>
                </tr>
                <tr>
                    <th class="required">제목</th>
                    <td colspan="5">
                        <input type="text" class="wid750" name="title" id="title" value="${futureEduReviewVo.title}"> <span class="co_red" id="txt_length">0</span>/50
                    </td>
                </tr>
                <tr>
                    ...
                    <td colspan="5">
                        ...
                        (파일형식: jpg, jpeg, bmp, gif / 사이즈 : <span class="co_red">450 * 305</span> )
                        <c:forEach items="${futureEduReviewVo.thumbFileList}" var="thumb" varStatus="status">
                            <div class="imgArea">
                            <span>
                ...
                <tr>
                    ...
                    <td colspan="5">
                        <input type="text" class="wid750" name="alt_txt" id="alt_txt" value="${futureEduReviewVo.alt_txt}">
                    </td>
                </tr>
                <tr>
                    <th>첨부파일</th>
                ...
                <tr>
                    ...
                    <td colspan="5">
                        <textarea name="cont" id="cont" class="textarea" cols="100%" rows="6" title="내용">${futureEduReviewVo.cont}</textarea>
                    </td>
                </tr>
                </tbody>
            </table>
        ...
        <div class="btnArea">
            <div class="left">
                <a href="/ssemadmin/cont/campus/fedu_review_list.do" class="btn">목록</a>
            </div>
            <div class="right">
