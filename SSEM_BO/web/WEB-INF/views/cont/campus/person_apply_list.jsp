            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/person_apply_list_excel.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/campus/person_apply_list.do">
        ...
        <article class="contUnit sch_area">
            ...
            <div class="line">
                <div class="formCont">
                    <label for="searchKind2">신청경로</label>
                ...
                <div class="formCont">
                    <label for="searchKind4">상태</label>
            ...
            <div class="line">
                <div class="formCont">
                    ...
                    <select id="searchVal2" name="searchVal2" class="wid100">
                ...
                <div class="formCont">
                    <select id="searchKind" name="searchKind" class="wid150">
                        <option value="ALL">전체</option>
                        <option value="1" <c:if test="${searchKind eq '1'}">selected="selected"</c:if>>인물도서 제목</option>
                        <option value="2" <c:if test="${searchKind eq '2'}">selected="selected"</c:if>>인물도서</option>
                        <option value="3" <c:if test="${searchKind eq '3'}">selected="selected"</c:if>>아이디</option>
                        <option value="4" <c:if test="${searchKind eq '4'}">selected="selected"</c:if>>신청자</option>
                        <option value="5" <c:if test="${searchKind eq '5'}">selected="selected"</c:if>>단체명</option>
                    </select>
        ...
        <table>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    ...
                    <td>${item.person}</td>
                    <td>${item.reg_id}</td>
                    <td><a href="/ssemadmin/cont/campus/person_apply_view.do?person_book_apy_idx=${item.person_book_apy_idx}">${item.apy_name}</a></td>
                    ...
                    <td>
                        <c:choose>
                            ...
                            <c:when test="${item.status eq 'P'}">신청조율</c:when>
                            <c:when test="${item.status eq 'O'}">열람확정</c:when>
