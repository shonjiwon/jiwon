    ...
    $(document).ready(function() {
        ...
        $("#btn_delete").click(function(){
            ...
            if(confirm("선택한 데이터를 삭제 하시겠습니까?.")) {
                ...
                $.ajax({
                    url: '/ssemadmin/ajax/new_fund_delete.do',
                    type: 'POST',
                    dataType: 'json',
                    data: data,
                    xhrFields: {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/new_fund_list.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
        ...
        $("#btn_insert").click(function(){
            location.href = "/ssemadmin/cont/self/new_fund_inscr.do?sdate=" + $('#searchEDate').val();
        });
    });
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/self/new_fund_list.do">
    ...
    <article class="contUnit tbl_area">
        ...
        <table>
            ...
            <tr style="background:#fdf9db;">
                ...
                <td>-</td>
                <td>-</td>
                <td class="ta_r"><fmt:formatNumber value="${tot_amount}" pattern="##,###"/></td>
                <td>-</td>
            </tr>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    <td><input type="checkbox" name="apply_id" id="apply_id_${item.fund_mng_seq}" value="${item.fund_mng_seq}"></td>
                    <td><fmt:formatNumber value="${totalCount - ((page - 1) * listSize + (lno - 1))}" pattern="##,###"/></td>
                    <td>${item.standard_yymm}</td>
                    <td>${item.dong_nm}</td>
                    <td>
                        <c:choose>
                            <c:when test="${item.div_cd eq '1'}">일반</c:when>
                            <c:when test="${item.div_cd eq '2'}">헬스장</c:when>
                        </c:choose>
                    </td>
                    <td class="ta_r"><a href="/ssemadmin/cont/self/new_fund_view.do?fund_mng_seq=${item.fund_mng_seq}"><fmt:formatNumber value="${item.amount}" pattern="##,###"/></a></td>
                    <td>${item.note}</td>
                </tr>
            </c:forEach>
            </tbody>
...
<section class="popArea pop_upload">
    <div class="popBody" style="width: 100rem !important;">
        ...
        <div class="cont">
            <article class="contUnit tbl_area">
                <form name="frm_excel" id="frm_excel" method="post" action="/ssemadmin/cont/excel/new_fund_excel_upload.do" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <a href="/ssemadmin/resources/download/fund_upload.xls" class="btn grey">양식다운로드</a>
                    </div>
                    <br/>
                    <table>
                        <colgroup>
