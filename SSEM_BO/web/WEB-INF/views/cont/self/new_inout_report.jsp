            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/new_inout_report.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/self/new_inout_report.do">
    ...
    <article class="contUnit sch_area">
        ...
        <div class="line">
            <div class="formCont">
                <label for="searchSDate">수출입일자</label>
                ...
                <label for="searchKind2">수입내역구분</label>
                <select id="searchKind2" name="searchKind2" class="wid150">
                    <option value="">전체</option>
                    <c:forEach items="${inCodeList}" var="incost" varStatus="status">
                        <option value="${incost.code_idx}" <c:if test="${searchKind2 eq incost.code_idx}">selected="selected"</c:if>>${incost.cd_val}</option>
                    </c:forEach>
                </select>
                <label for="searchKind3">지출내역구분</label>
                <select id="searchKind3" name="searchKind3" class="wid150">
                    <option value="">전체</option>
                    <c:forEach items="${exCodeList}" var="excost" varStatus="status">
                        <option value="${excost.code_idx}" <c:if test="${searchKind3 eq excost.code_idx}">selected="selected"</c:if>>${excost.cd_val}</option>
                    </c:forEach>
                </select>
    ...
    <article class="contUnit tbl_area">
        ...
        <table>
            ...
            <tbody>
            <tr>
                <td><fmt:formatNumber value="${prev_total_old + prev_total}" pattern="##,###"/></td>
                <td><fmt:formatNumber value="${in_total}" pattern="##,###"/></td>
                <td><fmt:formatNumber value="${ex_total}" pattern="##,###"/></td>
                <td><fmt:formatNumber value="${prev_total_old + prev_total + in_total - ex_total}" pattern="##,###"/></td>
                <td>-</td>
            </tr>
            </tbody>
        </table>
        ...
        <table>
            ...
            <c:forEach items="${inList}" var="in_item" varStatus="in_status">
                <tr>
                    ...
                    <td>${in_item.dong_nm}</td>
                    <td>${in_item.inout_cd_nm}</td>
                    <td><fmt:formatNumber value="${in_item.inout_cost}" pattern="##,###"/></td>
                    <td>${in_item.note}</td>
                </tr>
            </c:forEach>
            </tbody>
        ...
        <table>
            ...
            <c:forEach items="${exList}" var="ex_item" varStatus="ex_status">
                <tr>
                    ...
                    <td>${ex_item.dong_nm}</td>
                    <td>${ex_item.inout_cd_nm}</td>
                    <td><fmt:formatNumber value="${ex_item.inout_cost}" pattern="##,###"/></td>
                    <td>${ex_item.note}</td>
                </tr>
            </c:forEach>
            </tbody>
