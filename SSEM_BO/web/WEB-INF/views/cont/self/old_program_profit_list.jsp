            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/old_program_profit_list.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/self/old_program_profit_list.do">
    ...
    <article class="contUnit tbl_area">
        ...
        <table>
            ...
            <tr style="background:#fdf9db;">
                ...
                <td></td>
                <td><fmt:formatNumber value="${tot_collect_amount}" pattern="##,###"/></td>
                <td></td>
                <td><fmt:formatNumber value="${tot_support_amount}" pattern="##,###"/></td>
            </tr>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    ...
                    <td>${item.dong_cd}</td>
                    <td><a href="/ssemadmin/cont/self/old_program_profit_view.do?prog_result_seq=${item.prog_result_seq}">${item.prog_nm}</a></td>
                    <td>${item.man_cnt + item.woman_cnt}</td>
                    <td><fmt:formatNumber value="${item.lecturer_hourly_fee}" pattern="##,###"/></td>
                    <td><fmt:formatNumber value="${item.collect_amount}" pattern="##,###"/></td>
                    <td><fmt:formatNumber value="${item.lecturer_pay_amount}" pattern="##,###"/></td>
                    <td><fmt:formatNumber value="${item.support_amount}" pattern="##,###"/></td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
