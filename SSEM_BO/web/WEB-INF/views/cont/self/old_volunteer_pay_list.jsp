            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/old_volunteer_pay_list.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/self/old_volunteer_pay_list.do">
        ...
        <table>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${item.div_cd eq '10500001'}">회계담당</c:when>
                            <c:when test="${item.div_cd eq '10500002'}">야간개방등</c:when>
                            <c:when test="${item.div_cd eq '10500003'}">주말개방자 등</c:when>
                        </c:choose>
                    </td>
                    <td><a href="/ssemadmin/cont/self/old_volunteer_pay_view.do?service_cost_seq=${item.service_cost_seq}">${item.nm}</a></td>
                    <td>${item.resi_no}</td>
                    <td>${item.work_cnt}</td>
