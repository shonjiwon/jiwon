...
<script type="text/javascript">
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/new_teacher_pay_list.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
        ...
        $("#btn_insert").click(function(){
            location.href = "/ssemadmin/cont/self/new_teacher_pay_inscr.do?sdate=" + $('#searchEDate').val();
        });
    });
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/self/new_teacher_pay_list.do">
        ...
        <table>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    ...
                    <td><a href="/ssemadmin/cont/self/new_teacher_pay_view.do?lecturer_cost=${item.lecturer_cost}">${item.dong_nm}</a></td>
                    <td><fmt:formatNumber value="${item.lecture_cnt}" pattern="##,###"/></td>
                    <td><fmt:formatNumber value="${item.classroom_cnt}" pattern="##,###"/></td>
                    <td><fmt:formatNumber value="${item.teacher_cnt}" pattern="##,###"/></td>
                    <td><fmt:formatNumber value="${item.apply_amount}" pattern="##,###"/>원</td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
