            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/old_inout_report.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/self/old_inout_report.do">
    ...
    <article class="contUnit tbl_area">
        ...
        <table>
            ...
            <tbody>
            <tr>
                <td><fmt:formatNumber value="${prev_total}" pattern="##,###"/></td>
                <td><fmt:formatNumber value="${in_total}" pattern="##,###"/></td>
                <td><fmt:formatNumber value="${ex_total}" pattern="##,###"/></td>
                <td><fmt:formatNumber value="${prev_total + in_total - ex_total}" pattern="##,###"/></td>
                <td>-</td>
            </tr>
            </tbody>
        </table>
    ...
    <article class="contUnit tbl_area">
        ...
        <table>
            ...
            <c:if test="${fn:length(inList) == 0}">
                <tr>
                    <td colspan="5">데이터가 없습니다.</td>
                </tr>
            </c:if>
            <c:forEach items="${inList}" var="in_item" varStatus="in_status">
                <tr>
                    <td>${in_status.count}</td>
                    <td>${in_item.dong_cd}</td>
                    <td>
                        <c:choose>
                            <c:when test="${in_item.kind_cd eq '10800001'}">수강료</c:when>
                            <c:when test="${in_item.kind_cd eq '10800002'}">사용료</c:when>
                            <c:when test="${in_item.kind_cd eq '10800003'}">이자수입</c:when>
                            <c:when test="${in_item.kind_cd eq '10800004'}">기타</c:when>
                        </c:choose>
                    </td>
                    <td><fmt:formatNumber value="${in_item.amount}" pattern="##,###"/></td>
                    <td>${in_item.comments}</td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
    ...
    <article class="contUnit tbl_area">
        ...
        <table>
            ...
            <c:if test="${fn:length(exList) == 0}">
                <tr>
                    <td colspan="5">데이터가 없습니다.</td>
                </tr>
            </c:if>
            <c:forEach items="${exList}" var="ex_item" varStatus="ex_status">
                <tr>
                    <td>${ex_status.count}</td>
                    <td>${ex_item.dong_cd}</td>
                    <td>
                        <c:choose>
                            <c:when test="${ex_item.kind_cd eq '10900001'}">강사료</c:when>
                            <c:when test="${ex_item.kind_cd eq '10900002'}">장비구입/유지비</c:when>
                            <c:when test="${ex_item.kind_cd eq '10900003'}">시설비/보수비</c:when>
                            <c:when test="${ex_item.kind_cd eq '10900004'}">환불</c:when>
                            <c:when test="${ex_item.kind_cd eq '10900005'}">기타</c:when>
                        </c:choose>
                    </td>
                    <td><fmt:formatNumber value="${ex_item.amount}" pattern="##,###"/></td>
                    <td>${ex_item.comments}</td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
