...
<script type="text/javascript">
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/new_volunteer_pay_list.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
        ...
        $("#btn_insert").click(function(){
            location.href = "/ssemadmin/cont/self/new_volunteer_pay_inscr.do?sdate=" + $('#searchEDate').val();
        });
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/self/new_volunteer_pay_list.do">
        ...
        <table>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${item.div_cd eq '1'}">회계지원</c:when>
                            <c:when test="${item.div_cd eq '2'}">야간개방</c:when>
                            <c:otherwise>주말개방</c:otherwise>
                        </c:choose>
                    </td>
                    <td><a href="/ssemadmin/cont/self/new_volunteer_pay_view.do?service_cost_seq=${item.service_cost_seq}">${item.nm}</a></td>
                    <td>${item.resi_no}</td>
                    <td>${item.work_cnt}일</td>
                    <td><fmt:formatNumber value="${item.amount}" pattern="##,###"/>원</td>
                    <td>
                        <a href="/ssemadmin/cont/self/new_volunteer_pay_view.do?service_cost_seq=${item.service_cost_seq}" class="btn sml grey">수정</a>
                        <a href="/ssemadmin/cont/self/new_volunteer_pay_copy.do?service_cost_seq=${item.service_cost_seq}" class="btn sml grey">신규</a>
                    </td>
                </tr>
            </c:forEach>
            </tbody>
...
<section class="popArea pop_upload">
    <div class="popBody" style="width: 100rem !important;">
        ...
        <div class="cont">
            <article class="contUnit tbl_area">
                <form name="frm_excel" id="frm_excel" method="post" action="/ssemadmin/cont/excel/new_volunteer_pay_excel_upload.do" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <a href="/ssemadmin/resources/download/volunteer_upload.xls" class="btn grey">양식다운로드</a>
                    </div>
                    <br/>
                    <table>
                        <colgroup>
