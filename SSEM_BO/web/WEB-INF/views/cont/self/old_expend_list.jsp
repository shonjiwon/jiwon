            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/old_expend_list.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/self/old_expend_list.do">
        ...
        <table>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${item.kind_cd eq '10900001'}">강사료</c:when>
                            <c:when test="${item.kind_cd eq '10900002'}">장비구입/유지비</c:when>
                            <c:when test="${item.kind_cd eq '10900003'}">시설비/보수비</c:when>
                            <c:when test="${item.kind_cd eq '10900004'}">환불</c:when>
                            <c:when test="${item.kind_cd eq '10900005'}">기타</c:when>
                        </c:choose>
                    </td>
                    <td class="ta_r"><fmt:formatNumber value="${item.amount}" pattern="##,###"/></td>
                    <td class="ta_l"><a href="/ssemadmin/cont/self/old_expend_view.do?inout_seq=${item.inout_seq}">${item.break_down}</a></td>
                    <%--<td><a href="/ssemadmin/cont/excel/old_expend_report.do?inout_seq=${item.inout_seq}" class="btn sml grey">지출결의서</a></td>--%>
                </tr>
            </c:forEach>
            </tbody>
        </table>
