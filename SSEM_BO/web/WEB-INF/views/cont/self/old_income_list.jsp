...
<script type="text/javascript">
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/old_income_list.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
        ...
        $('#btn_income_report').click(function() {
            document.location.href="/ssemadmin/cont/excel/old_income_report.do?standard_date=" + $("#standard_date").val();
        });
    });
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/self/old_income_list.do">
    ...
    <article class="contUnit sch_area">
        ...
        <%--<div class="line">
            <div class="formCont">
                <label for="standard_date">일자</label>
                ...
                <select id="dong_kind" name="dong_kind" class="wid150" <c:if test="${adminLoginVo.type ne 'S'}">disabled</c:if>>
                    <option value="">전체</option>
                    <c:forEach items="${dongList}" var="dong" varStatus="status">
                        <option value="${dong.code}" <c:if test="${searchKind eq dong.code}">selected="selected"</c:if> <c:if test="${fn:contains(adminLoginVo.grp_name, dong.name)}">selected="selected"</c:if>>${dong.name}</option>
                    </c:forEach>
                </select>
                <a href="#none" class="btn blue" id="btn_income_report">수입결의서</a>
                &lt;%&ndash;<a href="#none" class="btn blue">출석부 다운로드</a>&ndash;%&gt;
            </div>
        </div>--%>
        <div class="line">
            <div class="formCont">
                <label for="searchSDate">일자</label>
        ...
        <table>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${item.kind_cd eq '10800001'}">수강료</c:when>
                            <c:when test="${item.kind_cd eq '10800002'}">사용료</c:when>
                            <c:when test="${item.kind_cd eq '10800003'}">이자수입</c:when>
                            <c:when test="${item.kind_cd eq '10800004'}">기타</c:when>
                        </c:choose>
                    </td>
                    <td class="ta_r"><fmt:formatNumber value="${item.amount}" pattern="##,###"/></td>
                    <td>${item.adj_yn}</td>
                    <td><a href="/ssemadmin/cont/self/old_income_view.do?inout_seq=${item.inout_seq}">${item.prog_nm}</a></td>
                    <td>${item.inout_nm}</td>
                    <td>
                        <c:choose>
                            <c:when test="${item.exemption eq '0'}">없음</c:when>
                            <c:when test="${item.exemption eq '1'}">50%</c:when>
                            <c:when test="${item.exemption eq '2'}">100%</c:when>
                        </c:choose>
                    </td>
                    <td><a href="/ssemadmin/cont/excel/course_report.do?inout_seq=${item.inout_seq}" class="btn sml grey">수강증</a></td>
                    <%--<td><a href="/ssemadmin/cont/excel/course_report.do?inout_seq=${item.inout_seq}" class="btn sml grey">출석부</a></td>--%>
                </tr>
            </c:forEach>
            </tbody>
        </table>
