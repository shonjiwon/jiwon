                ...
                $.ajax({
                    ...
                    success: function(data){
                        if (data !== true) {
                            ...
                            alert("삭제하였습니다.");
                            location.href = "?popup_idx=${popupVo.popup_idx}";
                        }
                    }
                });
            }else{
...
<div class="container">
    ...
    <article class="contUnit tbl_area leftType">
        ...
        <form name="delfrm" id="delfrm" action="/ssemadmin/cont/main/popup_delete.do" method="post" enctype="multipart/form-data">
            <input type="hidden" name="popup_idx" value="${popupVo.popup_idx}" />
        </form>
        <form name="frm" id="frm" action="/ssemadmin/cont/main/popup_view.do" method="post" enctype="multipart/form-data">
            <input type="hidden" name="popup_idx" id="popup_idx" value="${popupVo.popup_idx}" />
            <input type="hidden" name="atch_file_id" id="atch_file_id" value="${popupVo.atch_file_id}" />
            <table>
                <colgroup>
                    <col style="width:15%;">
                    <col style="width:35%;">
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <select id="gubun" name="kind" class="kind">
                            <option value="A" <c:if test="${popupVo.kind eq 'A'}">selected="selected"</c:if>>공통</option>
                            <option value="J" <c:if test="${popupVo.kind eq 'J'}">selected="selected"</c:if>>미래교육</option>
                            <option value="S" <c:if test="${popupVo.kind eq 'S'}">selected="selected"</c:if>>평생교육</option>
                        </select>
                    </td>
                </tr>
                <tr>
                    ...
                    <td colspan="3">
                        <input type="text" class="wid100per" name="name" id="name" value="${popupVo.name}">
                    </td>
                </tr>
                <tr>
                ...
                <tr>
                    <th>노출순서</th>
                    <td colspan="3">
                        <input type="text" class="wid100" name="ord" id="ord" value="${popupVo.ord}" oninput="this.value = this.value.replace(/[^0-9]/g, '');">
                    </td>
                </tr>
                <tr>
                    <th class="required">노출기간</th>
                    <td colspan="3">
                        <input type="text" id="start_dt" name="start_dt" class="wid150" value="${popupVo.start_dt}">
                        ...
                        ~
                        <input type="text" id="end_dt" name="end_dt" class="wid150" value="${popupVo.end_dt}">
                ...
                <tr>
                    ...
                    <td colspan="3">
                        ...
                        (파일형식: jpg, jpeg, bmp, gif, png / 사이즈 : <span class="co_red">400*500</span> )
                        <c:forEach items="${popupVo.fileList}" var="item" varStatus="status">
                            <div class="imgArea">
                            <span>
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <input type="text" class="wid750" name="alt_txt" id="alt_txt" value="${popupVo.alt_txt}">
                    </td>
                </tr>
                <tr>
                    <th>링크URL</th>
                    <td colspan="3">
                        <input type="text" class="wid500" name="link" id="link" value="${popupVo.link}">
                ...
                <tr>
                    <th>작성자</th>
                    <td>${popupVo.reg_id}</td>
                    <th>등록일</th>
                    <td>${popupVo.reg_dt}</td>
                </tr>
                <tr>
                    <th>최종수정자</th>
                    <td>${popupVo.up_id}</td>
                    <th>최종수정일</th>
                    <td colspan="3">${popupVo.up_dt}</td>
                </tr>
                </tbody>
            </table>
        </form>
