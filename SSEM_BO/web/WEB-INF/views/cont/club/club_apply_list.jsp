            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/club_apply_list_excel.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/club/club_apply_list.do">
    ...
    <article class="contUnit sch_area">
        ...
        <div class="line">
            <div class="formCont">
                <label for="searchKind2">분야 선택</label>
                <select id="searchKind2" name="searchKind2" class="wid150">
                    <option value="">전체</option>
                    <c:forEach items="${codeList}" var="item" varStatus="status">
                        <option value="${item.code_idx}" <c:if test="${item.code_idx eq searchKind2}">selected="selected"</c:if>>${item.cd_val}</option>
                    </c:forEach>
                </select>
            </div>
            <div class="formCont">
                <label for="searchKind3">처리상태</label>
        ...
        <div class="line">
            <div class="formCont">
                <label for="searchKind3">기간</label>
                <select id="searchKind4" name="searchKind4" class="wid150" title="기간">
                    <option value="">전체</option>
                    <option value="1" <c:if test="${searchKind4 eq '1'}">selected="selected"</c:if>>신청일</option>
                    <option value="2" <c:if test="${searchKind4 eq '2'}">selected="selected"</c:if>>최종수정일</option>
                </select>
            ...
            <div class="formCont">
                <label for="searchKind3">검색기준</label>
                <select id="searchKind" name="searchKind" class="wid150" title="검색기준">
                    <option value="ALL">전체</option>
                    <option value="1" <c:if test="${searchKind eq '1'}">selected="selected"</c:if>>동아리명</option>
                ...
                <input type="text" id="searchVal" name="searchVal" placeholder="검색어 입력" title="검색어" class="wid300" value="${searchVal}">
        ...
        <table>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    ...
                    <td>${item.code_nm}</td>
                    <td class="ta_l"><a href="/ssemadmin/cont/club/club_apply_view.do?club_idx=${item.club_idx}">${item.name}</a></td>
                    <td>${item.reg_id}</td>
                    <td>${item.apy_name}</td>
                    ...
                    <td>
                        <c:choose>
                            <c:when test="${item.status eq 'R'}">신청대기</c:when>
                            <c:when test="${item.status eq 'C'}">신청확인</c:when>
                            <c:when test="${item.status eq 'O'}">동아리확정</c:when>
                            <c:when test="${item.status eq 'N'}">신청취소</c:when>
