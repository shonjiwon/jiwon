                ...
                $.ajax({
                    ...
                    success: function(data){
                        if (data !== true) {
                            ...
                            alert("삭제하였습니다.");
                            location.href = "?club_news_idx=${clubNewsVo.club_news_idx}";
                        }
                    }
                });
            }else{
...
<div class="container">
    ...
    <article class="contUnit tbl_area leftType">
        ...
        <form name="delfrm" id="delfrm" action="/ssemadmin/cont/club/club_news_delete.do" method="post" enctype="multipart/form-data">
            <input type="hidden" name="club_news_idx" value="${clubNewsVo.club_news_idx}" />
        </form>
        <form name="frm" id="frm" action="/ssemadmin/cont/club/club_news_view.do" method="post" enctype="multipart/form-data">
            <input type="hidden" name="club_news_idx" id="club_news_idx" value="${clubNewsVo.club_news_idx}" />
            <input type="hidden" name="atch_file_id" id="atch_file_id" value="${clubNewsVo.atch_file_id}" />
        <table>
            <colgroup>
                <col style="width:15%">
                <col style="width:35%">
            ...
            <tr>
                <th>노출상태</th>
                <td colspan="5">
                    <input type="radio" name="use_yn" id="use_y" value="Y" <c:if test="${clubNewsVo.use_yn eq 'Y'}">checked="checked"</c:if>>
                    <label for="use_y">노출</label>
                    <input type="radio" name="use_yn" id="use_n" value="N" <c:if test="${clubNewsVo.use_yn eq 'N'}">checked="checked"</c:if>>
                    <label for="use_n">미노출</label>
                </td>
            </tr>
            <tr>
                <th>소속</th>
                <td>${clubNewsVo.p_name}</td>
                <th>교육기관</th>
                <td>${clubNewsVo.grp_name}</td>
            </tr>
            <tr>
                <th>작성자</th>
                <td colspan="3">${clubNewsVo.reg_id}</td>
            </tr>
            <tr>
                <th class="required">제목</th>
                <td colspan="3">
                    <input type="text" class="wid750" name="title" id="title" value="${clubNewsVo.title}">
                </td>
            </tr>
            <tr>
                <th>썸네일 파일</th>
                <td colspan="3">
                    ...
                    (파일형식: jpg, jpeg, bmp, gif, png / 사이즈 : <span class="co_red">335 * 225</span> )
                    <c:forEach items="${clubNewsVo.thumbFileList}" var="thumb" varStatus="status">
                        <div class="imgArea">
                            <span>
            ...
            <tr>
                <th>이미지대체텍스트</th>
                <td colspan="3">
                    <input type="text" class="wid750" name="alt_txt" id="alt_txt" value="${clubNewsVo.alt_txt}">
                </td>
            </tr>
            <tr>
                <th>첨부 파일</th>
            ...
            <tr>
                ...
                <td colspan="3">
                    <textarea name="cont" id="cont" class="textarea" cols="100%" rows="6" title="내용">${clubNewsVo.cont}</textarea>
                </td>
            </tr>
            </tbody>
        </table>
        ...
        <div class="btnArea">
            <div class="left">
                <a href="/ssemadmin/cont/club/club_news_list.do" class="btn">목록</a>
            </div>
            <div class="right">
