...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // 검색 조건중 소속 정보가 있는 경우
        let p_idx = '${bbsDetailVo.p_idx}';
        let group_idx = '${bbsDetailVo.group_idx}';
        if(p_idx !== '' && group_idx !== '') {
        ...
        $('#group_idx').change(function() {
            ...
            if(group_idx === "75" || group_idx === "76" || group_idx === "77" || group_idx === "78" || group_idx === "86") {
                $("#top_center").removeClass("dp_n");
            } else {
                $("#top_center").addClass("dp_n");
            }
        });
...
<div class="container">
    ...
    <article class="contUnit tbl_area leftType">
        ...
        <form name="delfrm" id="delfrm" action="/ssemadmin/cont/site/notice_delete.do" method="post" enctype="multipart/form-data">
            <input type="hidden" name="bbs_detail_idx" value="${bbsDetailVo.bbs_detail_idx}" />
        </form>
        <form name="frm" id="frm" action="/ssemadmin/cont/site/notice_view.do" method="post" enctype="multipart/form-data">
            ...
            <table>
                    ...
                    <td>
                        <select id="p_idx" name="p_idx">
                            <c:forEach items="${masterGroupList}" var="item" varStatus="status">
                                <option value="${item.group_idx}" <c:if test="${item.group_idx eq bbsDetailVo.p_idx}">selected="selected"</c:if>>${item.name}</option>
                            </c:forEach>
                        </select>
                    </td>
                ...
                <tr>
                    ...
                    <td colspan="5">
                        <input type="radio" name="p_top_yn" id="p_top_n" value="N" <c:if test="${bbsDetailVo.p_top_yn eq 'N'}">checked="checked"</c:if>>
                        <label for="top_n">일반</label>
                        <input type="radio" name="p_top_yn" id="p_top_y" value="Y" <c:if test="${bbsDetailVo.p_top_yn eq 'Y'}">checked="checked"</c:if>>
                        <label for="top_y">상단고정노출</label>
                    </td>
                </tr>
                <tr id="top_center" class="dp_n">
