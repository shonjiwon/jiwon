            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/apply_list_excel.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/lecture/apply_list.do">
        ...
        <div class="line">
            <div class="formCont">
                ...
                <c:forEach items="${tgtList}" var="tgt" varStatus="status">
                    <input type="checkbox" id="checkbox_${tgt.code_idx}"><label for="checkbox_${tgt.code_idx}">${tgt.cd_val}</label>
                </c:forEach>
            </div>
        </div>
        <div class="line">
            ...
            <div class="formCont">
                ...
                <select id="searchKind" name="searchKind" class="wid150">
                    ...
                    <option value="2" <c:if test="${searchKind eq '2'}">selected="selected"</c:if>>등록자</option>
                </select>
    ...
    <article class="contUnit tbl_area">
        ...
        <table>
            ...
            <tr>
                ...
                <th>신청방법</th>
                <th>접수기간</th>
                <th>정원<br>(인터넷(대기) | 방문 | 전화)</th>
                <th>모집현황<br>(인터넷(대기))</th>
                <th>등록자</th>
                <th>프로그램상태</th>
                <th>노출여부</th>
            </tr>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    ...
                    <td>${item.p_name}</td>
                    <td>${item.grp_name}</td>
                    <td class="ta_l"><a href="/ssemadmin/cont/lecture/apply_view.do?lecture_idx=${item.lecture_idx}">${item.name}</a></td>
                    <td>${item.start_dt} ~ ${item.end_dt}</td>
                    <td>${item.reg_method_str}</td>
                    <td>${fn:substring(item.reg_start_st, 0, 16)} ~ ${fn:substring(item.reg_end_dt, 0, 16)}</td>
                    <td>
                        <fmt:formatNumber value="${item.student_qty}" pattern="##,###"/>명<br>
                        (${item.online_qty}(${item.wait_qty}) | ${item.visit_qty} | ${item.tel_qty})
                    </td>
                    <td>
                        <fmt:formatNumber value="${(item.online_qty - item.online_can_qty) + (item.wait_qty - item.wait_can_qty)}" pattern="##,###"/>명<br>
                        (<c:out value="${item.online_qty - item.online_can_qty}" />(<c:out value="${item.wait_qty - item.wait_can_qty}" />))
                    </td>
                    <td>${item.reg_id}</td>
                    <td>
                        <c:choose>
