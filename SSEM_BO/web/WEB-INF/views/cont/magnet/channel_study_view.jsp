                ...
                $.ajax({
                    ...
                    success: function(data){
                        if (data !== true) {
                            ...
                            alert("삭제하였습니다.");
                            location.href = "?channel_study_idx=${channelStudyVo.channel_study_idx}";
                        }
                    }
                });
            }else{
...
<div class="container">
    ...
    <article class="contUnit tbl_area leftType">
        ...
        <form name="delfrm" id="delfrm" action="/ssemadmin/cont/magnet/channel_study_delete.do" method="post" enctype="multipart/form-data">
            <input type="hidden" name="channel_study_idx" value="${channelStudyVo.channel_study_idx}" />
        </form>
        <form name="frm" id="frm" action="/ssemadmin/cont/magnet/channel_study_view.do" method="post" enctype="multipart/form-data">
            <input type="hidden" name="channel_study_idx" id="channel_study_idx" value="${channelStudyVo.channel_study_idx}" />
            <table>
                <colgroup>
                    <col style="width:15%">
                    <col style="width:35%">
                ...
                <tr>
                    <th>노출상태</th>
                    <td colspan="3">
                        <input type="radio" name="use_yn" id="use_y" value="Y" <c:if test="${channelStudyVo.use_yn eq 'Y'}">checked="checked"</c:if>>
                        <label for="use_y">노출</label>
                        <input type="radio" name="use_yn" id="use_n" value="N" <c:if test="${channelStudyVo.use_yn eq 'N'}">checked="checked"</c:if>>
                        <label for="use_n">미노출</label>
                    </td>
                </tr>
                <tr>
                    <th>소속</th>
                    <td>${channelStudyVo.p_name}</td>
                    <th>교육기관</th>
                    <td>${channelStudyVo.grp_name}</td>
                </tr>
                <tr>
                    <th>작성자</th>
                    <td colspan="3">${channelStudyVo.reg_id}</td>
                </tr>
                <tr>
                    <th class="required">제목</th>
                    <td colspan="3">
                        <input type="text" class="wid750" name="title" id="title" value="${channelStudyVo.title}"> <span class="co_red" id="txt_length">0</span>/50
                    </td>
                </tr>
                <tr>
                    ...
                    <td colspan="3">
                        ...
                        (파일형식: jpg, jpeg, bmp, gif, png / 사이즈 : <span class="co_red">430 * 300</span> )
                        <c:forEach items="${channelStudyVo.thumbFileList}" var="thumb" varStatus="status">
                            <div class="imgArea">
                            <span>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" class="wid750" name="alt_txt" id="alt_txt" value="${channelStudyVo.alt_txt}">
                    </td>
                </tr>
                <tr>
                    <th>첨부파일</th>
                ...
                <tr>
                    ...
                    <td colspan="3">
                        <textarea name="cont" id="cont" class="textarea" cols="100%" rows="6" title="내용">${channelStudyVo.cont}</textarea>
                    </td>
                </tr>
                </tbody>
            </table>
        ...
        <div class="btnArea">
            <div class="left">
                <a href="/ssemadmin/cont/magnet/channel_study_list.do" class="btn">목록</a>
            </div>
            <div class="right">
