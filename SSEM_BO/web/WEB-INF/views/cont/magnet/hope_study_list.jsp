            ...
            $.ajax({
                ...
                success: function (data) {
                    if (data !== 0) {
                        ...
                        document.location.href="/ssemadmin/cont/excel/hope_study_list_excel.do?" + param;
                    } else {
                        alert("등록에 실패하였습니다.");
                    }
                }
...
<div class="container">
    ...
    <form name="frm" id="frm" method="post" action="/ssemadmin/cont/magnet/hope_study_list.do">
            ...
            <div class="line">
                <div class="formCont">
                    ...
                    <select id="searchKind2" name="searchKind2" class="wid150">
                        ...
                        <option value="1" <c:if test="${searchKind2 eq '1'}">selected="selected"</c:if>>등록일</option>
                        <option value="2" <c:if test="${searchKind2 eq '2'}">selected="selected"</c:if>>청원마감일</option>
                    </select>
                ...
                <div class="formCont">
                    <label for="searchKind3">진행상태</label>
                    <select id="searchKind3" name="searchKind3" class="wid150">
                        <option value="">전체</option>
                        <option value="1" <c:if test="${searchKind3 eq '1'}">selected="selected"</c:if>>진행중</option>
                        <option value="2" <c:if test="${searchKind3 eq '2'}">selected="selected"</c:if>>종료</option>
                    </select>
                </div>
                <div class="formCont">
                    <label for="searchKind4">선정여부</label>
                    <select id="searchKind4" name="searchKind4" class="wid150">
                        <option value="">전체</option>
                        <option value="Y" <c:if test="${searchKind4 eq 'Y'}">selected="selected"</c:if>>선정</option>
                        <option value="N" <c:if test="${searchKind4 eq 'N'}">selected="selected"</c:if>>미선정</option>
                    </select>
                </div>
            </div>
            <div class="line">
                <div class="formCont">
                    ...
                    <select id="searchKind" name="searchKind" class="wid150">
                        ...
                        <option value="2" <c:if test="${searchKind eq '2'}">selected="selected"</c:if>>이름</option>
                    </select>
        ...
        <table>
            ...
            <c:forEach items="${itemList}" var="item" varStatus="status">
                ...
                <tr>
                    ...
                    <td><a href="/ssemadmin/cont/magnet/hope_study_view.do?hope_study_idx=${item.hope_study_idx}">${item.name}</a></td>
                    <td>${item.apy_name}</td>
                    <td>${item.apy_id}</td>
                    <td>${item.apy_mob_no}</td>
                    <td>${item.grp_name}(${item.grp_cnt})</td>
                    <td>${item.join_cnt}</td>
                    <td>${item.teach_cnt}</td>
                    <td>${fn:substring(item.reg_dt, 0, 10)}</td>
                    <td>${item.close_dt}</td>
                    <td>
                        <c:choose>
                            <c:when test="${item.close_yn eq 'Y'}">종료</c:when>
                            <c:otherwise>진행중</c:otherwise>
                        </c:choose>
                    </td>
                    <td>
                        <c:choose>
                            <c:when test="${item.select_yn eq 'Y'}">선정</c:when>
                            <c:when test="${item.select_yn eq 'N'}">미선정</c:when>
