...
<script type="text/javascript">
    $(document).ready(function() {
        $(document).prop('title', '온라인 상담예약 > 진학학습지원센터 > 나의 송파쌤 > 평생교육 > 송파쌤');
    ...
    <div class="sub_con st_5 mypage_st"><!-- sub_con -->
        ...
        <div class="tab_box fs_20 fs_md_26"><!-- tab_box -->
            ...
            <ul class="tab tab_7"> <!-- tab_3 ~ tab_8 :  min : 3개 ~  max : 8개 -->
                ...
                <li class="tab_item on"><a href="/ssem/senior/mypage/online_counsel_list.do"><span class="tab_middle">진학학습지원센터</span></a></li>
        ...
        <div class="mypage_tab_box fs_20 fs_md_26"><!-- mypage_tab : s -->
            <ul class="tab tab_1">
                <li class="tab_item on"><a href="/ssem/senior/mypage/online_counsel_list.do">온라인 상담예약</a></li>
            </ul>
        </div><!-- mypage_tab : end -->
        <div class="data_search ptb_30 fs_20  fs_md_26 fs_md_24"><!--Data Search-->
            <form name="frm" id="frm" method="post" action="/ssem/senior/mypage/online_counsel_list.do">
                <input type="hidden" name="page" id="page" value="${page}"/>
                <div class="wrap_1250 plr_40">
                    <div class="row_24">
                        <div class="col_12 col_md_12 pr_20">
                            <!--
                                sel_box select class="placeholder"
                                [option value= ""] placeholder 컬러
                            -->
                            <div class="sel_box sel_full">
                                <select id="searchKind3" name="searchKind3" title="희망상담유형 선택" class="placeholder">
                                    <option value="">희망상담유형 선택</option>
                        ...
                        <div class="col_12 col_md_12 pr_20 pr_md_0">
                            <div class="sel_box sel_full">
                                <select id="searchKind2" name="searchKind2" title="예약상태" class="placeholder">
                    ...
                    <div class="row_24">
                        <div class="col_8 col_md_24 ">
                            <div class="sel_box sel_full">
                                <select id="searchVal2" name="searchVal2" title="기간" class="placeholder">
                                    <option value="">기간 선택</option>
                                    <option value="1" <c:if test="${searchVal2 eq '1'}">selected="selected"</c:if>>희망상담일</option>
                                    <option value="2" <c:if test="${searchVal2 eq '2'}">selected="selected"</c:if>>신청일</option>
                                    <option value="3" <c:if test="${searchVal2 eq '3'}">selected="selected"</c:if>>상담예약일시</option>
                                </select>
                            </div>
                        </div>
                        <div class="col_16 col_md_24 pl_20 pl_md_0 line_md_top">
                            <div class="row_24">
                                <div class="col_10">
                                    <div class="fm_cal">
                    ...
                    <ul>
                        <li class="t_head fs_20">
                            <section class="table">
                                ...
                                <div class="col wid_250">희망상담 유형</div>
                                <div class="col wid_250">희망상담일</div>
                                <div class="col wid_250">신청일</div>
                                <div class="col wid_250">상담예약 일시</div>
                                <div class="col wid_250">예약상태</div>
                            </section>
                        </li>
                        <!-- item_box :s -->
                        <c:if test="${fn:length(itemList) == 0}">
                            <li class="_moInit noDate">
                                <div class="t_center">
                                    ...
                                    <p  class="mt_30 fs_30"><span class="fw_600">‘온라인 상담예약’</span> 에 대한 검색결과가 없습니다.</p>
                                </div>
                            </li>
                        </c:if>
                        ...
                        <c:forEach items="${itemList}" var="item" varStatus="status">
                            ...
                            <li class="wa_ba">
                                <a href="/ssem/senior/mypage/online_counsel_insert.do?self_counsel_idx=${item.self_counsel_idx}">
                                    <section class="table">
                                        ...
                                        <div class="col wid_250">${item.meet_dt}</div>
                                        <div class="col wid_250">${fn:substring(item.reg_dt, 0, 10)}</div>
                                        <div class="col wid_250">
                                            <c:choose>
                                                <c:when test="${item.resv_dt ne null}">${item.resv_dt}</c:when>
                                                <c:otherwise>-</c:otherwise>
                                            </c:choose>
                                        </div>
                                        <div class="col wid_250">
                                            <c:choose>
                                                <c:when test="${item.status eq 'A'}"><span class="my_tc_2">예약접수</span></c:when>
                                                <c:when test="${item.status eq 'R'}"><span class="my_tc_1">예약완료</span></c:when>
                                                <c:when test="${item.status eq 'O'}"><span class="my_tc_4">상담완료</span></c:when>
                                                <c:when test="${item.status eq 'C'}"><span class="my_tc_3">예약취소</span></c:when>
                                                <c:otherwise>-</c:otherwise>
                                            </c:choose>
                                        </div>
                                    </section>
