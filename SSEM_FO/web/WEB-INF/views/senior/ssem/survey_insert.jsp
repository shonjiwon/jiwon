...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#btn_insert").click(function(e) {
            var check = false, len = 0, txt = "";
            // 완료시 validation 체크
            <c:forEach items="${svyMstVo.questList}" var="quest" varStatus="status_quest">
                <c:choose>
                <c:when test="${quest.type eq 'S'}">
                check = $('input:radio[name="questList[${status_quest.index}].select_item_idx"]').is(':checked');
                if(!check){ alert("미 기재 항목이 있습니다.\n모든 문항에 답변을 부탁드립니다."); return false;}
                </c:when>
                <c:when test="${quest.type eq 'M'}">
                len = $('input:checkbox[name="questList[${status_quest.index}].select_item_idx"]:checked').length
                if(len == 0){ alert("미 기재 항목이 있습니다.\n모든 문항에 답변을 부탁드립니다."); return false;}
                </c:when>
                <c:otherwise>
                txt = $('#item_${status_quest.index}').val();
                if("" == txt) { alert("미 기재 항목이 있습니다.\n모든 문항에 답변을 부탁드립니다."); return false;}
                </c:otherwise>
                </c:choose>
            </c:forEach>
...
<div class="contents" id="contents">
    ...
    <div class="sub_con st_5"><!-- sub_con -->
        ...
        <div class="wrap text_con fs_20 fs_md_26 mt_50 mb_15 mb_100">
            ...
            <div class="table_box fs_18 fs_md_24"><!--table_box-->
                <form name="frm_reply" id="frm_reply" action="/ssem/senior/ssem/survey_insert.do" method="post" enctype="multipart/form-data">
            ...
            <div class="btn_box mt_50 mb_100 pb_30 t_right t_md_center">
                ...
                <a href="/ssem/senior/ssem/survey_list.do" class="btn btn_short t_center">목록</a>
            </div>
        </div>
    </div>
</div>
