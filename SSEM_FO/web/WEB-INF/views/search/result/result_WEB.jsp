...
<c:set var = "thisCollection" value="ssem_webpage"/>
<c:set var = "thisTotalCount" value="${search._WEB[0].COUNT}"/>

<c:if test="${search.collection == 'ALL' || search.collection == thisCollection}">

	<c:if test="${thisTotalCount gt 0 }">
		<div class="s_item s_list_box">
			<h2 class="s_tit">웹페이지 (총 <span class="tc_p"><c:if test="${search._WEB[0].COUNT eq null}">
						0
					</c:if>
					<c:if test="${search._WEB[0].COUNT ne null}">
						${search._WEB[0].COUNT}
					</c:if></span>건)</h2>
			<ul class="s_list">
					<c:forEach items="${search._WEB}" var="idx" begin="1">
						<li>
							<a href="${idx.WEB_URL }" class="s_list_tit" target="_blank">${idx.TITLE }</a>
							<ul class="s_loaction_nav"> 

								<li class="home"></li>
								<li><a href="${idx.WEB_URL }" target="_blank">${idx.CONTENT }</a></li>
							</ul>
						</li>
					</c:forEach>
	...
	<c:if test="${search.collection eq 'ALL'}">
		<c:if test="${thisTotalCount eq 0 || thisTotalCount eq null}">
			<div class="s_item s_list_box">
				<h2 class="s_tit">웹페이지 (총 <span class="tc_p">0</span>건)</h2>
