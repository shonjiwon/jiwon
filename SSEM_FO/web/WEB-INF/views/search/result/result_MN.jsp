...
<c:set var = "thisCollection" value="ssem_menusch"/>
<c:set var = "thisTotalCount" value="${search._MN[0].COUNT}"/>
...
<c:if test="${search.collection == 'ALL' || search.collection == thisCollection}">
	...
	<c:if test="${thisTotalCount gt 0 }">
		<div class="s_item s_menu">
		
				<h2 class="s_tit">메뉴검색 (총 <span class="tc_p"><c:if test="${search._MN[0].COUNT eq null}">
							0
						</c:if>
						<c:if test="${search._MN[0].COUNT ne null}">
							${search._MN[0].COUNT}
						</c:if></span>건)</h2>
				<ul class="s_menu">
					
						<c:forEach items="${search._MN}" var="idx" begin="1">
						<li><a href="${idx.MENU_URL }" target="_blank">${idx.CONTENT }</a></li>			
			        	</c:forEach>	
	...
	<c:if test="${search.collection eq 'ALL'}">
		<c:if test="${thisTotalCount eq 0 || thisTotalCount eq null}">
			<div class="s_item s_list_box">
				<h2 class="s_tit">메뉴검색 (총 <span class="tc_p">0</span>건)</h2>
